...
if [[ $ROLLBACK -eq 1 ]]; then
  ...
  if [[ -d "$persist_base/latest-prev" ]]; then
    ...
    echo "No previous snapshot to rollback" >&2
    exit 1
  fi
fi
...
if [[ -n "$sig_b64" ]]; then
  if [[ -n "${SOLEN_SIGN_PUBKEY_PEM:-}" && -n "$OPENSSL" ]]; then
    sig_file="$work/manifest.sig"
    printf '%s' "$sig_b64" | base64 -d > "$sig_file"
    ...
    echo "Signature required but verification not possible (missing pubkey or openssl)" >&2
    exit 4
  else
