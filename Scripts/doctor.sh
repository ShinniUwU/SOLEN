...
if command -v ufw > /dev/null 2>&1; then
  fw_kind=ufw
  ufw status 2> /dev/null | grep -qi '^status: active' && fw_enabled=true
elif command -v nft > /dev/null 2>&1; then
  fw_kind=nftables
  nft list ruleset 2> /dev/null | grep -q 'table' && fw_enabled=true
elif command -v iptables > /dev/null 2>&1; then
  fw_kind=iptables
  iptables -S 2> /dev/null | grep -q '^-P' && fw_enabled=true
fi
