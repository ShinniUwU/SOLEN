...
val() {
  ...
  local key="$1"
  awk -v k="^[[:space:]]*${key}[[:space:]]" 'BEGIN{v="unset"} /^[[:space:]]*#/ {next} $0~k{v=$2} END{print v}' "$conf"
}
...
if printf '%s' "$ciphers" | grep -qiE '(arcfour|rc4|des)'; then
  weak=$((weak + 1))
  summary_parts+=("weak-cipher")
fi
if printf '%s' "$macs" | grep -qiE '(hmac-md5|umac-96)'; then
  weak=$((weak + 1))
  summary_parts+=("weak-mac")
fi
if printf '%s' "$kex" | grep -qiE 'diffie-hellman-group1-sha1'; then
  weak=$((weak + 1))
  summary_parts+=("weak-kex")
fi

status="ok"
[[ $weak -gt 0 ]] && status="warn"
